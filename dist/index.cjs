...
function injectEventMethod(classReflection, checker, mapType, eventName, params) {
	const method = new typedoc.DeclarationReflection(eventName, typedoc.ReflectionKind.Method, classReflection);
	method.setFlag(typedoc.ReflectionFlag.Public, true);
	const jsDoc = getEventJsDoc(mapType, eventName);
	const summaryText = jsDoc.summary ?? `Emitted when the \`${eventName}\` event fires.`;
	method.comment = new typedoc.Comment([{
		kind: "text",
		text: summaryText
	}]);
	method.comment.blockTags.push(new typedoc.CommentTag("@group", [{
		kind: "text",
		text: "Events"
	}]));
	method.comment.blockTags.push(new typedoc.CommentTag("@event", [{
		kind: "text",
		text: eventName
	}]));
	if (jsDoc.remarks) method.comment.blockTags.push(new typedoc.CommentTag("@remarks", [{
		kind: "text",
		text: jsDoc.remarks
	}]));
	if (jsDoc.deprecated) method.comment.blockTags.push(new typedoc.CommentTag("@deprecated", [{
		kind: "text",
		text: jsDoc.deprecated
	}]));
	if (jsDoc.since) method.comment.blockTags.push(new typedoc.CommentTag("@since", [{
		kind: "text",
		text: jsDoc.since
	}]));
	const sig = new typedoc.SignatureReflection(eventName, typedoc.ReflectionKind.CallSignature, method);
	sig.comment = new typedoc.Comment([{
		kind: "text",
		text: summaryText
	}]);
	sig.parameters = params.map((p) => {
		const param = new typedoc.ParameterReflection(p.name, typedoc.ReflectionKind.Parameter, sig);
		param.setFlag(typedoc.ReflectionFlag.Optional, p.optional);
		param.type = {
			type: "intrinsic",
			name: p.typeStr
		};
		sig.comment.blockTags.push(new typedoc.CommentTag("@param", [{
			kind: "text",
			text: p.typeStr
		}]));
		return param;
...
function load(app) {
	app.converter.on("createDeclaration", (context, reflection) => {
		if (reflection.kind !== typedoc.ReflectionKind.Class) return;
